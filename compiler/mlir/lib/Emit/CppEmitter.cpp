...
static void emitWireProbes(llvm::raw_ostream &os, Type, unsigned w,
...
static LogicalResult emitCombAssign(Operation &op, llvm::raw_ostream &os, NameTable &nt) {
  ...
  if (auto priority = dyn_cast<pyc::PriorityEncodeOp>(op)) {
    unsigned inputWidth = bitWidth(priority.getIn().getType());
    unsigned indexWidth = bitWidth(priority.getIndex().getType());
    if (inputWidth == 0 || indexWidth == 0)
      return priority.emitError("invalid priority_encode width");
  ...
  if (auto popcount = dyn_cast<pyc::PopcountOp>(op)) {
    unsigned inputWidth = bitWidth(popcount.getIn().getType());
    unsigned outputWidth = bitWidth(popcount.getCount().getType());
...
static LogicalResult emitFunc(func::FuncOp f, llvm::raw_ostream &os, const CppEmitterOptions &opts) {
  ...
  for (Operation *op : ordered) {
    ...
    if (isa<pyc::ConstantOp, pyc::AddOp, pyc::SubOp, pyc::MulOp, pyc::UdivOp,
            ...
            pyc::ShlOp, pyc::LshrOp, pyc::AshrOp, arith::SelectOp>(*op)) {
      if (failed(emitCombAssign(*op, os, nt)))
        return failure();
      continue;
    }
