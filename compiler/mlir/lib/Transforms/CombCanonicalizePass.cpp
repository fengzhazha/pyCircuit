...
static Value constInt(PatternRewriter &rewriter, Location loc, Type ty,
                      const llvm::APInt &v) {
  auto intTy = dyn_cast<IntegerType>(ty);
  if (!intTy)
    return {};
  llvm::APInt value = v;
  if (value.getBitWidth() != intTy.getWidth())
    value = value.zextOrTrunc(intTy.getWidth());
  return rewriter.create<pyc::ConstantOp>(loc, intTy,
                                          IntegerAttr::get(intTy, value));
}

static bool isI1(Type ty) {
  auto intTy = dyn_cast<IntegerType>(ty);
...
struct MuxSameSelSimplify : public OpRewritePattern<pyc::SelectOp> {
  ...
  LogicalResult matchAndRewrite(pyc::SelectOp op,
    ...
    if (auto aMux = op.getA().getDefiningOp<pyc::SelectOp>()) {
      if (aMux.getSel() == sel) {
        rewriter.replaceOpWithNewOp<pyc::SelectOp>(op, op.getType(), sel,
                                                   aMux.getA(), op.getB());
        return success();
      }
    }
    ...
    if (auto n = sel.getDefiningOp<pyc::NotOp>()) {
      rewriter.replaceOpWithNewOp<pyc::SelectOp>(op, op.getType(), n.getIn(),
                                                 op.getB(), op.getA());
      return success();
    }
...
struct MuxI1ToLogic : public OpRewritePattern<pyc::SelectOp> {
  ...
  LogicalResult matchAndRewrite(pyc::SelectOp op,
    ...
    if (bConst && (((*bConst) & 1) == 0)) {
      // sel ? a : 0  ==>  sel & a
      rewriter.replaceOpWithNewOp<pyc::AndOp>(op, op.getResult().getType(),
                                              op.getSel(), op.getA());
      return success();
    }
    if (aConst && (((*aConst) & 1) == 1)) {
      // sel ? 1 : b  ==>  sel | b
      rewriter.replaceOpWithNewOp<pyc::OrOp>(op, op.getResult().getType(),
                                             op.getSel(), op.getB());
      return success();
    }
    if (bConst && (((*bConst) & 1) == 1)) {
      ...
      rewriter.replaceOpWithNewOp<pyc::OrOp>(op, op.getResult().getType(), nsel,
                                             op.getA());
      return success();
    }
...
struct AndBasicSimplify : public OpRewritePattern<pyc::AndOp> {
  ...
  LogicalResult matchAndRewrite(pyc::AndOp op,
                                PatternRewriter &rewriter) const override {
    ...
    if ((lb == rb) && (ln != rn)) {
      Value z =
          constInt(rewriter, op.getLoc(), op.getType(), llvm::APInt(1, 0));
      if (!z)
        return failure();
...
struct OrBasicSimplify : public OpRewritePattern<pyc::OrOp> {
  ...
  LogicalResult matchAndRewrite(pyc::OrOp op,
                                PatternRewriter &rewriter) const override {
    Value lhs = stripAlias(op.getLhs());
    Value rhs = stripAlias(op.getRhs());
    ...
    auto [rb, rn] = stripNot(rhs);
    if ((lb == rb) && (ln != rn)) {
      Value ones = constInt(rewriter, op.getLoc(), op.getType(),
                            llvm::APInt::getAllOnes(
                                isa<IntegerType>(op.getType())
                                    ? cast<IntegerType>(op.getType()).getWidth()
                                    : 1));
      if (!ones)
        return failure();
...
struct OrAndXorFactor : public OpRewritePattern<pyc::OrOp> {
  ...
  LogicalResult matchAndRewrite(pyc::OrOp op,
    ...
    for (auto [A, B] : cands) {
      // XOR: (A & ~B) | (~A & B) ==> A ^ B
      if (andMatches(a0, A, /*aNot=*/false, B, /*bNot=*/true) &&
          andMatches(a1, A, /*aNot=*/true, B, /*bNot=*/false)) {
        rewriter.replaceOpWithNewOp<pyc::XorOp>(op, op.getType(), A, B);
        return success();
      }
      if (andMatches(a0, A, /*aNot=*/true, B, /*bNot=*/false) &&
          andMatches(a1, A, /*aNot=*/false, B, /*bNot=*/true)) {
        rewriter.replaceOpWithNewOp<pyc::XorOp>(op, op.getType(), A, B);
        return success();
      }

      // XNOR: (A & B) | (~A & ~B) ==> ~(A ^ B)
      if (andMatches(a0, A, /*aNot=*/false, B, /*bNot=*/false) &&
          andMatches(a1, A, /*aNot=*/true, B, /*bNot=*/true)) {
        Value x = rewriter.create<pyc::XorOp>(op.getLoc(), op.getType(), A, B);
        rewriter.replaceOpWithNewOp<pyc::NotOp>(op, op.getType(), x);
        return success();
      }
      if (andMatches(a0, A, /*aNot=*/true, B, /*bNot=*/true) &&
          andMatches(a1, A, /*aNot=*/false, B, /*bNot=*/false)) {
        Value x = rewriter.create<pyc::XorOp>(op.getLoc(), op.getType(), A, B);
        rewriter.replaceOpWithNewOp<pyc::NotOp>(op, op.getType(), x);
        return success();
      }
...
struct CombCanonicalizePass
    : public PassWrapper<CombCanonicalizePass, OperationPass<func::FuncOp>> {
  ...
  StringRef getDescription() const override {
    return "Simplify combinational PYC logic (mux canonicalization and small "
           "boolean rewrites)";
  }
  ...
  void runOnOperation() override {
    ...
    patterns.add<MuxSameSelSimplify, MuxI1ToLogic, AndBasicSimplify,
...
std::unique_ptr<::mlir::Pass> createCombCanonicalizePass() {
  return std::make_unique<CombCanonicalizePass>();
}
