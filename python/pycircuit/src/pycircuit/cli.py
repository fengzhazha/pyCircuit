...
def _tool_script(name: str) -> Path:
    candidates = [
        ...
        Path(__file__).resolve().parents[4] / "flows" / "tools" / name,
    ]
...
def _cmd_build(args: argparse.Namespace) -> int:
    ...
    if do_cpp:
        ...
        library_source_include = Path(__file__).resolve().parents[4] / "library"
        if library_source_include.is_dir():
            include_dirs.append(str(library_source_include))
