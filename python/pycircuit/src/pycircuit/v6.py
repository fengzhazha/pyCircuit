"""PyCircuit V6 cycle-aware frontend (tutorial + Cycle-Aware API).
...
# Union of every signal flavor accepted by the V6 coercion helpers
# (CycleAwareSignal.as_cas / is_cas, mux, cat). Forward
...
class CycleAwareCircuit(Circuit):
    """V6 top-level builder; extends Circuit so m.out / m.cat / emit_mlir work unchanged."""

    def __init__(self, name: str, design_ctx: Any | None = None) -> None:
        super().__init__(name, design_ctx=design_ctx)
        self._v6_design: Any | None = None

    def emit_mlir(self) -> str:
        if self._v6_design is not None:
    ...
    def delay_to(
        ...
        *,
        ...
        for _ in range(d):
            ...
            nm = f"_v6_bal_{self._delay_serial}"
            r = self._m.out(
...
class StateSignal(Generic[DT]):
    ...
    Exposes the same ``.ty`` / ``.width`` / ``.signed`` inspection surface as :class:`Wire`.
    """
    ...
    @property
    def ty(self) -> DT:
        """Underlying scalar data type (matches :class:`Wire.ty`)."""
        return self._cas.ty

    @property
    def width(self) -> int:
        """Scalar bit width (matches :class:`Wire.width`)."""
...
class ForwardSignal(Generic[DT]):
    ...
    Exposes the same ``.ty`` / ``.width`` / ``.signed`` inspection surface as
    :class:`Wire` so callers can inspect the represented scalar type.
...
class CycleAwareSignal(Generic[DT]):
    ...
    @property
    def width(self) -> int:
        """Scalar bit width, matching :class:`Wire`."""
...
def _strip_domain_for_jit(
    ...
    exec(compile(ast.parse(new_src), "<pycircuit_v6_strip_domain>", "exec"), globs)
    out: Callable[..., Any] = globs[name]
    out.__pycircuit_jit_source__ = new_src
    out.__pycircuit_jit_start_line__ = 1
    out.__pycircuit_jit_source_file__ = "<pycircuit_v6_strip_domain>"
...
# ---------------------------------------------------------------------------
# V6 Cycle-Aware Testbench wrapper
# ---------------------------------------------------------------------------
...
class CycleAwareTb:
    ...
    def drive(self, port: str, value: int | bool) -> None:
        """Drive a scalar port at the current cycle."""
        self._t.drive(port, int(value), at=self._cycle)
    ...
    def expect(
        ...
        *,
        ...
        """Check a scalar port at the current cycle."""
        self._t.expect(port, int(value), at=self._cycle, phase=phase, msg=msg)
